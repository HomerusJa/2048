...
use crate::sliding::{calculate_row_left_to_right_slide, invert_row, row_to_tiles};
...
impl Board {
    ...
    pub fn get(&self, x: u8, y: u8) -> u32 {
        // TODO: This struct should only work with exponents, not actual values. The conversion between the two should be done in the UI layer, not in the game logic.
    ...
    #[inline(always)]
    pub fn get_row(&self, row: u8) -> u16 {
        assert!(row < 4, "Row out of bounds");
        ((self.board >> (row * 16)) & 0xFFFF) as u16
    }

    pub fn get_row_tiles(&self, row: u8) -> [u32; 4] {
        assert!(row < 4, "Row out of bounds");
        let row_value = self.get_row(row);
        let tiles = row_to_tiles(row_value);
        [
            if tiles[0] == 0 { 0 } else { 1 << tiles[0] },
            if tiles[1] == 0 { 0 } else { 1 << tiles[1] },
            if tiles[2] == 0 { 0 } else { 1 << tiles[2] },
            if tiles[3] == 0 { 0 } else { 1 << tiles[3] },
        ]
    }

    pub fn get_row_tiles_exponents(&self, row: u8) -> [u8; 4] {
        assert!(row < 4, "Row out of bounds");
        let row_value = self.get_row(row);
        row_to_tiles(row_value)
    }
    ...
    ///
    /// Note that the least significant bit corresponds to the top-left corner, and the
    /// most significant bit corresponds to the bottom-right corner. This is so you can
    /// access the bits the following way: `mask & (1 << (x + y * 4)) != 0` to check if
    /// the tile at (x,y) is empty.
    #[inline(always)]
    pub fn empty_mask(&self) -> u16 {
        let mut mask = 0u16;
        for i in 0..16 {
            if self.get_exponent_by_index(i) == 0 {
                mask = mask | (1 << i);
            }
        }
        mask
...
mod tests {
    ...
    #[test]
    fn test_single_tile() {
        let board = Board::default().with_tile(0, 0, 1);
    ...
    fn test_overwrite_tile() {
        let board = Board::default().with_tile(2, 1, 1).with_tile(2, 1, 5);
    ...
    #[test]
    fn test_clear_tile() {
        let board = Board::default().with_tile(1, 2, 4).with_tile(1, 2, 0);
    ...
    fn test_roundtrip_all_cells() {
        for i in 0..16 {
            let board = Board::default().with_tile_by_index(i, 7);
    ...
    fn test_empty_mask_some_filled() {
        ...
        // Note that the least significant bit corresponds to the top-left corner, and
        // the most significant bit corresponds to the bottom-right corner.
        assert_eq!(mask, 0b1111_1111_1101_1110);
    }
    ...
    fn test_basic_slide_with_score() {
        let board = Board::default().with_tile(0, 0, 1).with_tile(1, 0, 1);

        let (new_board, score) = board.slide_with_score(Direction::Right).unwrap();

        let expected_board = Board::default().with_tile(3, 0, 2);
    ...
    fn test_slide_right_moves_row_into_place() {
        let board = Board::default().with_tile(0, 2, 1).with_tile(2, 2, 2);

        let new_board = board.slide(Direction::Right).unwrap();

        let expected_board = Board::default().with_tile(2, 2, 1).with_tile(3, 2, 2);
        assert_eq!(new_board, expected_board);
    }

    #[test]
    fn test_slide_up_basic() {
        let board = Board::default().with_tile(1, 1, 1).with_tile(1, 3, 1);

        let (new_board, score) = board.slide_with_score(Direction::Up).unwrap();

        let expected_board = Board::default().with_tile(1, 0, 2);
        assert_eq!(new_board, expected_board);
        assert_eq!(score, 4);
    }
    ...
    fn test_slide_down_basic() {
        let board = Board::default().with_tile(2, 0, 1).with_tile(2, 2, 1);

        let (new_board, score) = board.slide_with_score(Direction::Down).unwrap();

        let expected_board = Board::default().with_tile(2, 3, 2);
        assert_eq!(new_board, expected_board);
        assert_eq!(score, 4);
    }
    ...
    fn test_slide_left_basic() {
        let board = Board::default().with_tile(3, 1, 1).with_tile(1, 1, 1);

        let (new_board, score) = board.slide_with_score(Direction::Left).unwrap();

        let expected_board = Board::default().with_tile(0, 1, 2);
        assert_eq!(new_board, expected_board);
        assert_eq!(score, 4);
    }
    ...
    #[test]
    fn test_transpose() {
        let board = Board::default().with_tile(1, 3, 1);
